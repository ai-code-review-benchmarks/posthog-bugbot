...
class TestWebAnalyticsVsTrendsComparison(ClickhouseTestMixin, APIBaseTest):
    ...
    def test_single_session_multiple_pageviews_same_path(self):
        ...
        # Both systems should report the same values in this simple case
            ...
            [
                (
                    ...
                    [
                        ...
                        ("2024-01-10T14:00:00Z", session2, "/home"),  # New session
    ...
    def test_web_stats_table_session_grouping_effect(self):
        ...
        self._create_pageview_events(
            [
                (
                    ...
                    [
                        ("2024-01-10T10:00:00Z", session_id, "/home"),
                        ("2024-01-10T10:01:00Z", session_id, "/home"),  # Same path
                        ("2024-01-10T10:02:00Z", session_id, "/about"),
                        ("2024-01-10T10:03:00Z", session_id, "/about"),  # Same path
                        ("2024-01-10T10:04:00Z", session_id, "/about"),  # Same path
                    ],
                ),
            ]
        )
        ...
        # Web stats table returns rows per path
        # Each row has (path, (visitors_current, visitors_prev), (views_current, views_prev))
        ...
        # Both should count all pageviews
    ...
    def test_multiple_users_same_session_id_edge_case(self):
        ...
        # Web Analytics groups by session_id first, using any(person_id)
        # But uniq() in outer query should still count both unique persons
        # Note: The inner query groups by (session_id, breakdown_value), so with shared
        # session_id and same path, we get one row with any(person_id) picking one person
        # randomly. The outer uniq() will only see that one person.
        # This is a known limitation when session_ids are incorrectly shared.
        ...
        # The key insight: with shared session_id, web analytics may undercount visitors
        # because the inner GROUP BY (session_id, breakdown_value) uses any(person_id)
        # This is expected behavior - session_ids should be unique per user session
    ...
    @parameterized.expand(
        [
            # (test_name, num_users, pageviews_per_user, sessions_per_user)
    ...
    def test_parameterized_comparison(self, _name, num_users, pageviews_per_session, sessions_per_user):
        ...
        # Both systems should agree on total pageviews
        ...
        # Both systems should agree on unique users
        ...
        # Web Analytics tracks sessions, Trends does not by default
...
class TestWebAnalyticsVsTrendsQuerySnapshots(ClickhouseTestMixin, APIBaseTest):
    ...
    def test_snapshot_web_overview_query(self):
        ...
        Key characteristics to note in the query:
        - Joins with sessions table
        - Groups by session first in subqueries
        - Uses uniq() for visitor counting
        """
    ...
    def test_snapshot_web_stats_table_query(self):
        ...
        - MAIN_INNER_QUERY groups by (session_id, breakdown_value)
        - Uses any(person_id) per session
        - count() gives pageviews per session per path
        """
        self._create_sample_events()
    ...
    def test_snapshot_trends_unique_users_query(self):
        ...
        Key characteristics:
        - Uses count(DISTINCT person_id)
        - No session grouping
        - Direct aggregation on events
        """
        self._create_sample_events()
    ...
    def test_snapshot_trends_unique_sessions_query(self):
        ...
        This is the Trends equivalent of Web Analytics session counting.

        Key characteristics:
        - Uses count(DISTINCT $session_id)
        - No session table join needed
        """
        self._create_sample_events()
